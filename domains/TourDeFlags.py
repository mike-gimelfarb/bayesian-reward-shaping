...
# this domain implementation is a simplified version
...
class TourDeFlags(Domain):
    ...
    def __init__(self, maze, initial):
        ...
        self.max_steps = max(300, self.goals * (self.width + self.height))
    ...
    def state_dim(self):
        return self.width + self.height + self.goals + 1
    ...
    def render(self, policy, encoder, time=0.02):
        ...
        while not done and steps < self.max_steps:
            ...
            if state == new_state:
                break
            row, col, collected = state = new_state
            grid[row, col] = 0.25
            if not im:
